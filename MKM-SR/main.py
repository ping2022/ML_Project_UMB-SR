...
if __name__ == '__main__':
    ...
    parser.add_argument('--remove_new_items', type = bool, default = True, help='whether remove new items:if add argument,it will be true') #if add argument,it will be true,no new item
   ...
   #    dataset += 'with_new_item/'
    ...
    if opt.mode == 'MKM_SR':
        ...
        for index, item_id in enumerate(item_ids):
            #dictionary of item_id : item_id-1
        ...
        for epoch in range(opt.epoch):
            ...
            if bad_counter >= opt.patience:
                break
        output_data = pd.DataFrame({"hit": hit_list,
                                    "mrr": mrr_list,
                                    "current epoch": epoch_list})
        output_data.plot(x='current epoch', y=['hit', 'mrr'])
        plt.show()
        print('-------------------------------------------------------')
